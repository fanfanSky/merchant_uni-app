<template>
	<view class="tui-container" @touchmove.stop.prevent="stop">
		<view class="imageCropper" @touchend="cutTouchEnd" @touchstart="cutTouchStart" @touchmove="cutTouchMove">
...
export default {
	...
	props: {
		//defaultCropper.vue传过来的本地未裁剪的图片路径
		imageUrl: {  
			type: String,
			default: ''
		},
		...
		*/
		...
		rotateImg: {
			type: String,
			default: '/static/imageCropper/img_rotate.png'
		}
	},
	data() {
		return {
	...
	mounted() {
		...
		this.ctx = uni.createCanvasContext('imageCropper', this);
		this.setCutCenter();
	...
	methods: {
		//返回裁剪后图片信息
		getImage() {
			console.log("我是没有裁剪的图片---->",this.imageUrl)
			if (!this.imageUrl) {
				uni.showToast({
					title: '请选择图片',
					icon: 'none'
				...
				this.ctx.draw(false, () => {
					...
					if (this.isBase64) {
						this.ctx.toDataURL(params).then(dataURL => {
							...
							console.log("data.base64的图片",dataURL)
						});
					} else {
						this.ctx.toTempFilePath({
							...
							success: res => {
								...
								this.loadding && uni.hideLoading();
								this.$emit('cropper', data);
								console.log("测试看看是不是裁剪的图",data.url)
							}
						});
					}
					// #endif
		...
		cropperImage(){
			console.log("裁剪回传图片地址",this.imageUrl)
		},
		/**
		 * 设置剪裁框和图片居中
		 */
