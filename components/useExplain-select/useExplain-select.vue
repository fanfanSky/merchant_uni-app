<template>
	<div class="show-box"   :class="{disabled: disabled, active: isShowList}" :style="style_Container">
		<!-- 输入框，仅在可输入模式下使用 -->
		<input  
			v-if="showInput"  class="input" disabled="disable" placeholder-style="color: #bbb;"
		...
		<!-- 列表框 -->
		<div class="list-container" style="background-color: #FFFFFF;border: 1px solid #dcdfe6;box-shadow: 0 2px 12px 0 rgba(0,0,0,.1);border-radius: 8px;"  @click.stop="onListClick"
		:style="'top:' + listTop__ + 'px;'" v-show="isShowList">
			...
			>
				...
				<div v-show="innerList.length==0" class="data-state item" style="text-align: center;">无更多数据</div>
				<slot></slot>
			</scroll-view>
			<div @click="toAddNewTemplate()" class="data-state item addNewTemplate">添加新模板</div>
		</div>
		<!-- 下拉框标题对应的内容 -->
		<div></div>
	</div>
...
<script>
	...
	export default {
		...
		props: {
			...
			initTemplatesName:'',
			// 初始化模板内容
			initTemplatesContent:'',
			routeID:'', // 来源页面。1.新增商品 2.编辑商品
			goodsType:'', // 类型。1.代金券 2.套餐
			goodInfo:'', // 商品信息
		...
		computed:{
			...
			innerList(){    // 转换列表的数据格式
				...
				// 	aryList.push(orginArr[i].templatesName)
				// }
				// console.log("转换列表的数据格式aryList",aryList)
				// this.list =  aryList
		...
		methods: {
			...
			init(){
				...
				this.setInput(this.initTemplatesName); // 在输入框中显示初始值
				this.changeActiveIndex(this.initTemplatesName); // 根据初始值设置列表框中的活动项
			...
			onFocus(event,index){
				...
				console.log("监控事件==>",this.selectText)  // 这个银行卡名称跟卡号差了一个索引，不太准确，但是要传的数据(卡号)和在页面上显示的是同步的，这个可以不管
			},
			...
			onClickItem(index, value){  // 列表项上的点击事件
				...
				if( !this.itemIsActive(index) ){  //如果点在非选中项上
					...
					this.$emit('changeItem', {newVal: value, oldVal: this.selectText, 
											index: index, orignItem: this.list[index]});
					this.setInput(value);   // 更改输入框的值
					console.log('我被点击中啦',value)
				}
			},
			onListHide(){ //列表隐藏时的回调
				this.isRotate = false;
