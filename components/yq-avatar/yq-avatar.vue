<template name="yq-avatar">
	<view>
		<view>
			<view class='sunsin_picture_item sunsin_width' v-show="imageLength < count"
			 v-if="upImgConfig.iconReplace =='' || upImgConfig.iconReplace==undefined">
				<view class="sunsin_add_image sunsin_width" @click='fSelect' :src="imgSrc.imgSrc" :style="'background-color:'+upImgConfig.upBgColor+''"
				 v-show="!upImgConfig.isAddImage || false">
					...
					<span class="iconimg-text"  v-if="routID == 1" >点击修改头像</span>
					<view class="icon-text" v-else :style="'color:'+upImgConfig.upIconColor+';width:100%;'">{{upImgConfig.upTextDesc==undefined ||upImgConfig.upTextDesc==''?'上传图片':upImgConfig.upTextDesc}}</view>
