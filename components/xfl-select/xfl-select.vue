	...
	<div class="show-box"  :class="{disabled: disabled, active: isShowList}" :style="style_Container">
		...
		:style="'top:' + listTop__ + 'px;'" v-show="isShowList">
			...
			>
				...
				>
					<div>{{item.value}}</div>
					<div v-show="false">{{cardCodeLists[index]}}</div>
				</div>
				<div v-show="innerList.length==0" class="data-state item" style="text-align: center;">无更多数据</div>
				<!-- <div @click="toAddNewTemplate()"  class="data-state item" style="font-size:14px;color:rgba(10,30,44,1);text-align: center;">添加新模板</div> -->
				<slot>
				</slot>
			</scroll-view>
		</div>
...
<script>
	...
	export default {
		...
		created() {
			// console.log("我是什么页面===========",this.msg)
			// console.log("我是什么页面===========",this.list)
		},
		data() {
			return {
				...
				cardCode:''        // 拿到的需要传给后台的数据，再传回extractMoney.vue中
			};
		},
		...
		methods: {
			...
			onInput(event){
				...
				this.cardCode = inputVal
				// 这个cardCode值是不选银行卡时,选择输入银行卡号的时候传给extractMoney.vue的
				this.$emit("cardCode",this.cardCode)
				console.log("输入时的值=>",this.cardCode)
			},
			...
			onClickItem(index, value){  // 列表项上的点击事件
				...
				// 这里给父组件extractMoney.vue传卡号(cardCode)值
				this.cardCode = this.cardCodeLists[index]
				console.log("哈哈哈哈哈===>",this.cardCode)
				this.$emit('cardCode',this.cardCode)
			},
			onListHide(){ //列表隐藏时的回调
				this.isRotate = false;
